param([Parameter(Mandatory=$true)][string]$BackupPath,[string]$DatabasePath=$env:SANTA_OPS_DB_PATH,[string]$ServicePidFile=$env:SANTA_OPS_PID_FILE)
...
if($ServicePidFile-and(Test-Path -LiteralPath $ServicePidFile)){$servicePid=[int](Get-Content -LiteralPath $ServicePidFile -Raw).Trim();if(Get-Process -Id $servicePid -ErrorAction SilentlyContinue){throw "Stop the Santa Ops service before restore"}}
