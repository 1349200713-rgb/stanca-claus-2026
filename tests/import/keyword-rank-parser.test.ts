...
test("accepts Excel serial dates and rejects impossible calendar dates", () => {
  expect(parseKeywordRankRows([{ 日期: 46358, 关键词: "Santa", ASIN: "B0CFPYYPRN", 自然排名: 8 }]).records[0]?.date).toBe("2026-12-02");
  expect(parseKeywordRankRows([{ 日期: "2026-99-99", 关键词: "Santa", ASIN: "B0CFPYYPRN", 自然排名: 8 }]).records).toEqual([]);
});
